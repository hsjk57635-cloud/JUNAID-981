    ...
    try {
        ...
        if (format === 'video') {
            ...
            return await sock.sendMessage(chatId, {
                ...
                mimetype: 'video/mp4',
                fileName: `${title.replace(/[\\/:*?"<>|]/g, '').slice(0, 70)}.mp4`,
                caption: `🎬 *${title}*\n\n> *_JUNAID-MD_³⁰²_*`
            }, { quoted: message });
        }

        // Music = normal audio; Voice = WhatsApp voice-note (PTT)
        let audioPayload;
        try {
            const local = await downloadAudioLocal(video.url);
            localFile = local.path;
            audioPayload = require('fs').readFileSync(localFile);
        } catch (localErr) {
            console.log(`[PLAY] local ${format} download failed, using API chain:`, localErr.message);
            const data = await downloadAny(video.url, 'mp3');
            audioPayload = { url: data.downloadUrl };
        }

        return await sock.sendMessage(chatId, {
            audio: audioPayload,
            mimetype: 'audio/mpeg',
            fileName: `${title.replace(/[\\/:*?"<>|]/g, '').slice(0, 70)}.mp3`,
            ptt: format === 'voice'
        }, { quoted: message });
    } finally {
        if (localFile) {
...
async function handlePlayButtonResponse(sock, message, context = {}) {
    try {
        ...
        let params = {};
        try { params = JSON.parse(native.paramsJson || '{}'); } catch (_) {}
        const id = String(params.id || params.selectedId || params.buttonId || '');

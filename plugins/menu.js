...
const menuCommand = {
    ...
    async handler(sock, message, args, context) {
        ...
        try {
            ...
            const menuMessage = await sock.sendMessage(chatId, { text: menuText, ...channelInfo }, { quoted: message });

            // Small reaction on the menu message (Android/WhatsApp style).
            // This does not alter the menu text, video, or sound.
            try {
                if (menuMessage?.key) {
                    await sock.sendMessage(chatId, { react: { text: '🐍', key: menuMessage.key } });
                }
            } catch (reactionError) {
                console.warn('[MENU] Reaction skipped:', reactionError.message);
            }
